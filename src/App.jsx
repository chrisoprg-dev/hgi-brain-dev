  ...
  useEffect(() => {
    ...
    vec3 volumetricGlow(vec3 ro, vec3 rd){
      ...
      for(int i=0;i<40;i++){
        ...
        if(dist < 0.5){
          float intensity = exp(-dist * 3.0) * 0.04;
          ...
          vec3 col = mix(
            ...
            vec3(0.4, 0.1, 0.9),  // purple
            smoothstep(-1.0, 1.0, p.y)
          );
          col += vec3(0.3, 0.7, 1.0) * fire * 3.0; // cyan neural fires
          col += vec3(1.0, 0.5, 0.2) * pow(fire, 2.0) * 2.0; // orange hotspots
    ...
    vec3 platformGlow(vec3 ro, vec3 rd){
      ...
      if(t > 0.0){
        ...
        for(float i=1.0;i<7.0;i++){
          ...
          float intensity = exp(-ringDist * 40.0) * (0.3 - i * 0.03);
    ...
    void main(){
      ...
      if(d < MAX_DIST){
        ...
        if(brain < SURF_DIST * 2.0){
          ...
          col += vec3(0.2, 0.5, 1.0) * fresnel * 2.0;
          col += vec3(1.0) * pow(fresnel, 6.0) * 1.0;
        }
      }
